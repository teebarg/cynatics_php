 ...
 * @package App\Helpers
 *
 * @author: Jacob Ayokunle <jacob.ayokunle@supermart.ng>
 */
class Helper
{
     ...
     * @throws \Exception
     */
    public static function generateRandom($length, $keyspace = '0A1B2C3D4E5F6G7H8I9JKLMNOPQRSTUVWXYZ')
    {
        //hold pieces of number characters in array
        $pieces = [];
    ...
    public static function getStrippedDownStackTrace(\Throwable $e) {
        ...
        foreach ($trace as $key => &$elem) {
            foreach ($elem['args'] as $key => &$elemArg) {
                if (is_object($elemArg) && !$elemArg instanceof \App\Models\BaseModel &&
    ...
    public static function groupBy(array $input, $criteria)
    {
        return array_reduce($input, function ($prev, $next) use ($criteria) {
            $key = gettype($criteria) === 'object' ? $criteria($next) :  $next[$criteria];
            $prev[$key][] = $next;
            return $prev;
        }, []);
    }
